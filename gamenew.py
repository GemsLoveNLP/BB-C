...
# todo config here -------------
DIFF = {'NORMAL': {'wait': 1.2, 'max': 2, 'max_wait': 90}, 'HARD': {'wait': 0.8, 'max': 4, 'max_wait': 60}}
    ...
    for y in range(GRID):
        for x in range(GRID):
            ...
            if main_board.board[y][x].status != 0:
                icon_surface = pygame.image.load(f'{main_board[y][x].status}.png')
...
def find_correct():
    ...
    for correct_color in correct_list:
        for y in range(GRID):
            for x in range(GRID):
                if main_board.board[y][x].color == correct_color['text_color'] and get_color(x, y) == correct_color['text']:
                    # print(x,y)
...
def get_pred_out(pred_color, pred_shape):

    color_ind = ['b', 'r', 'y', 'v', 'g', 'o', 'p', 'w', 'm']
    shape_ind = [1, 4, 3, 2] #"circle, pie, pill, hex"
    i, j = 0, 0
    out_l = [[], [], [], [], [], []]
    for c, s in zip(pred_color, pred_shape):
        if c != 9 and s != 4:
            out_l[j].append(((color_ind[c], shape_ind[s])))
        else:
            out_l[j].append(("0", 0))
...
def main_game():
    global main_board, player_in_game, DIFFICULTY, pred_out
    ...
    while True:
        ...
        if status == 'begin':
            ...
            if wait % 60 == 59:
                check_player = c.req(np.array([1]))["c"]
                print(check_player)
                ...
                if check_player[4] == 1:
                    ...
                    p2.set_status(True)
                    join_key = True
                if check_player[35] == 1:
                    pill_animation_status = False
                    p3.set_status(True)
                    join_key = True
                if check_player[30] == 1:
                    hex_animation_status = False
                    p4.set_status(True)
                    join_key = True

                # if all(check_player[i] != 1 for i in (0,4,30,35)) and join_key == True:
                #     print("done selecting player")
                #     beginner_animation_status = True
                #     expert_animation_status = True
                #     player_in_game = []
                #     for pi in [p1, p2, p3, p4]:
            ...
            if wait > CIRCLE_THRESHOLD:
                pred_shape = c.req(np.array([0]))["s"]
                print(pred_shape)
                status = 'animate'
                wait = 0

                pred_out = get_pred_out(pred_color, pred_shape)
                print(pred_out)
            else:
                # print('white')
            ...
            animate_winner(find_correct(), animation_count,
                           size=SIZE)  # !find_correct_grid() has to be changed to find_correct
            animation_count += 1
        elif status == 'score':
